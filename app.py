...
def handle_new_members(message):
    ...
    for member in message.new_chat_members:
        ...
        if member.id != bot.get_me().id and member.id != adder_id:
            added_users += 1

    if added_users > 0:
        new_total = increment_added_count(adder_id, chat_id, added_users)
        if new_total >= 5:
            bot.send_message(chat_id, f"🎉 Badhai ho {adder_name}! Aapne {new_total} members add kar diye hain. Ab aap group me message kar sakte hain.")
        else:
...
# Har ek message par ye check hoga ki user ne 5 member add kiye hain ya nahi
...
def handle_all_messages(message):
    ...
    # Agar 5 se kam member add kiye hain toh message delete kar do
    if count < 5:
        try:
